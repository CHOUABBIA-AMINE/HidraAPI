...
| HDP-004 | Blocked | Owner affirmed authorization for project dataset use on 2026-09-21 and explicitly excluded `data/Fiche Passation.xlsx` from import; its `BadZipFile` no longer blocks in-scope work. HyFloAPI Actions run #35577651574 supplied structural metadata for 12 in-scope workbooks and eight SQL dumps. A further read-only schema sample of `hyflo_v01.sql` found 56 opaque coded tables and eight distinct basic column types but no defensible domain semantics. In-scope source business content, individual dataset eligibility, version precedence, appropriate confidentiality/security review and G1 remain unresolved. No records approved for import. |
...
**Current task: HDP-004 — Blocked pending controlled in-scope source-content review, dataset-specific import eligibility, suitable security review and canonical-version decisions.** The source-data owner asserted project-use authority on 2026-09-21 and excluded `Fiche Passation.xlsx`; no workbook repair is required for this workstream. Structural metadata extraction succeeded in HyFloAPI Actions run #35577651574, but does not authorize HDP-005 or imports.
