 ...
 *
 * @Description : WorkflowTaskId value object.
 *
 */
package dz.sh.hidra.modules.workflow.domain.value;
 ...
 */
public record WorkflowTaskId(String value) implements ValueObject {
    ...
    public static WorkflowTaskId newId() {
        return new WorkflowTaskId(UUID.randomUUID().toString());
    }

    private static String normalize(String value) {
        if (value == null || value.isBlank()) {
            throw new InvalidValueObjectException("WorkflowTaskId must not be null or blank.");
        }

        String normalized = value.trim();

        if (normalized.length() > 80) {
            throw new InvalidValueObjectException("WorkflowTaskId length must not exceed 80 characters.");
        }
