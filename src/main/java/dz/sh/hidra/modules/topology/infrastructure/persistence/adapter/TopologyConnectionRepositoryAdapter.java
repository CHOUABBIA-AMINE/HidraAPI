 ...
 *
 * @Name        : TopologyConnectionRepositoryAdapter
 ...
 *
 * @Description : Persistence adapter implementing TopologyConnectionRepositoryPort.
 *
 */
package dz.sh.hidra.modules.topology.infrastructure.persistence.adapter;
...
public final class TopologyConnectionRepositoryAdapter implements TopologyConnectionRepositoryPort {
    ...
    public TopologyConnection save(TopologyConnection topologyConnection) {
        Objects.requireNonNull(topologyConnection, "TopologyConnection must not be null.");
        return mapper.toDomain(jpaRepository.save(mapper.toEntity(topologyConnection)));
    }

    @Override
    public Optional<TopologyConnection> findById(TopologyConnectionId id) {
        Objects.requireNonNull(id, "TopologyConnectionId must not be null.");
    ...
    public PageResult<TopologyConnection> findAll(ListTopologyConnectionsQuery query) {
        ...
        List<TopologyConnection> filteredItems = jpaRepository.findAll().stream()
    ...
    private static boolean matchesSearchText(TopologyConnectionJpaEntity entity, String searchText) {
        if (searchText == null) {
            return true;
        }
