/**
 *
 * @Project     : HidraAPI
 * @Product     : Hidra - Hydrocarbon Intelligence for Data, Risk, and Analytics
 * @Author      : Abir MEDJERAB
 * @Owner       : Sonatrach / TRC : Digitalization Initiative
 *
 * @Name        : RecordAuditEventUseCase
 * @CreatedOn   : 2025-06-26
 * @UpdatedOn   : 2026-06-11
 *
 * @Type        : Interface
 * @Layer       : Application
 * @Module      : audit
 * @Package     : dz.sh.hidra.modules.audit.application.port.in
 *
 * @Description : Use case for recording audit events.
 *
 */
package dz.sh.hidra.modules.audit.application.port.in;

import dz.sh.hidra.modules.audit.application.command.RecordAuditEventCommand;
import dz.sh.hidra.modules.audit.application.dto.AuditEventSummaryDto;

/**
 * Use case for recording audit events.
 */
public interface RecordAuditEventUseCase {

    AuditEventSummaryDto recordAuditEvent(RecordAuditEventCommand command);
}
