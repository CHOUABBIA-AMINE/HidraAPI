 ...
 *
 * @Description : Domain event published when an identity user is activated.
 *
 */
package dz.sh.hidra.modules.identity.domain.event;
...
/**
 * Domain event published when an identity user is activated.
 *
 * <p>Business role: records that a previously registered or suspended identity user is
 * now active and eligible for role-based access evaluation.</p>
 *
 ...
 * secret value.</p>
 *
 * <p>Usage: create with {@link #newEvent(UserId, Instant)} when a user activation
...
public record UserActivatedEvent(
    ...
    private static final String EVENT_TYPE = "identity.user.activated";
    ...
    public static UserActivatedEvent newEvent(UserId userId, Instant occurredAt) {
        return new UserActivatedEvent(DomainEventId.newId(), occurredAt, userId);
    }
