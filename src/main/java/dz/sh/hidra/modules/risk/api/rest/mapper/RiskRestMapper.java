...
public final class RiskRestMapper {
    ...
    public static CreateRiskRegisterCommand toCommand(CreateRiskRegisterRequest request) {
        return new CreateRiskRegisterCommand(request.code(), request.nameAr(), request.nameFr(), request.nameEn(), request.description(), request.registerTypeId(), request.ownerOrganizationUnitId(), request.ownerOrganizationUnitNameSnapshot(), request.scopeType(), request.scopeId(), request.scopeCodeSnapshot(), request.scopeLabelSnapshot(), request.reviewFrequencyId(), request.effectiveFrom(), request.effectiveTo(), request.createdByActorId(), request.createdByDisplayNameSnapshot());
    }

    public static CreateRiskAssessmentCommand toCommand(CreateRiskAssessmentRequest request) {
        return new CreateRiskAssessmentCommand(request.riskRegisterId(), request.assessmentNumber(), request.title(), request.description(), request.assessmentTypeId(), request.methodologyId(), request.scopeId(), request.riskScenarioId(), request.assessmentDate(), request.validFrom(), request.validTo(), request.assessedByActorId(), request.assessedByDisplayNameSnapshot());
    }

    public static RiskRegisterResponse toResponse(RiskRegisterSummaryDto dto) {
        return new RiskRegisterResponse(dto.id(), dto.code(), dto.nameFr(), dto.registerTypeId(), dto.scopeType(), dto.scopeId(), dto.status());
    }

    public static RiskAssessmentResponse toResponse(RiskAssessmentSummaryDto dto) {
        return new RiskAssessmentResponse(dto.id(), dto.riskRegisterId(), dto.assessmentNumber(), dto.title(), dto.riskScenarioId(), dto.status(), dto.inherentScore(), dto.inherentRatingId(), dto.residualScore(), dto.residualRatingId(), dto.assessmentDate());
    }
}
