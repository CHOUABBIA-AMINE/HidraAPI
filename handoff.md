...
Active continuation branch created from current `main`:
...
Implemented on the HWEB-004 branch.
...
```text
HidraWEB/docs/roadmap/HidraAPI-Frontend-Backend-Gap-Register.md
```
...
Current `HidraSecurityConfiguration` supports:
...
13. Update HidraWEB gap register item-by-item with exact evidence.
14. Only then begin HWEB-005.
```

## 8. Backend implementation standards

Read `AGENTS.md` and the relevant module roadmap before changing a module.
...
Do not claim success without executing the repository gates.
...
## 10. HidraWEB rules before HWEB-005
...
- discovered that PR #51 was already merged into `main`;
- created fresh branch `frontend-backend-gap-remediation-02` from current `main`;
...
Do not restart HWEB-005 and do not recreate topology contracts that already exist.
...
```text
[ ] Every confirmed gap in the HidraWEB register is re-audited against the final HidraAPI SHA.
...
[ ] `mvn -q clean verify` passes, with Testcontainers status reported accurately.
[ ] `HidraAPI-Frontend-Backend-Gap-Register.md` is updated with final evidence and exact backend commit.
[ ] Only after the above does HWEB-005 begin.
```

---

This file is a handoff artifact, not a substitute for `AGENTS.md`, module roadmaps, or `HidraAPI-Frontend-Backend-Gap-Register.md`. When they differ, re-audit the current repository state and follow the authoritative repository instructions.