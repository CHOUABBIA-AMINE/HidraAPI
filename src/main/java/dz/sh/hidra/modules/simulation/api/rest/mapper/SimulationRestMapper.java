...
public final class SimulationRestMapper {
    ...
    public static CreateSimulationModelCommand toCommand(CreateSimulationModelRequest request) {
        return new CreateSimulationModelCommand(request.code(), request.nameAr(), request.nameFr(), request.nameEn(), request.modelTypeId(), request.topologyScopeType(), request.topologyScopeId(), request.description());
    }

    public static CreateSimulationScenarioCommand toCommand(CreateSimulationScenarioRequest request) {
        return new CreateSimulationScenarioCommand(request.code(), request.nameAr(), request.nameFr(), request.nameEn(), request.scenarioTypeId(), request.modelId(), request.modelVersionId(), request.topologySnapshotId(), request.planningReferenceId(), request.monitoringContextId(), request.createdByActorId(), request.createdByDisplayNameSnapshot());
    }

    public static SimulationModelResponse toResponse(SimulationModelSummaryDto dto) {
        return new SimulationModelResponse(dto.id(), dto.code(), dto.nameFr(), dto.modelTypeId(), dto.topologyScopeType(), dto.status(), dto.createdAt());
    }

    public static SimulationScenarioResponse toResponse(SimulationScenarioSummaryDto dto) {
        return new SimulationScenarioResponse(dto.id(), dto.code(), dto.nameFr(), dto.scenarioTypeId(), dto.modelId(), dto.modelVersionId(), dto.topologySnapshotId(), dto.status(), dto.createdAt());
    }
}
